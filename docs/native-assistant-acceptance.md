...
2026-09-20：Xcode 已从旧副本切换到本仓库工程，签名团队与原 App 一致；1.0（构建 2）已覆盖安装到 MO-iPhone 并成功启动。更新前 App 容器备份约12MB，位于仓库外的本机私有备份目录；更新后 `rihou.days.v1` 与备份一致，旧版没有 `rihou.todos.v1`，没有清空或覆盖既有待办数据。服务访问码待用户填写，真实日记上传同意与 iOS 通知许可仍由用户操作；本记录不代表下方所有真机交互和通知测试均已完成。
