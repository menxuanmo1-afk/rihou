...
// Native integrations observe committed storage only; the browser has no listener.
function changed(detail = {}) {
  if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent("rihou:store-changed", { detail }));
}
...
export function saveDay(day) {
  const all = readJson(DAYS, {});
  const previous = loadDay(day.date);
...
export function clearFuturePlanInstances(seriesId, fromIso) {
  ...
  if (changed) { writeJson(DAYS, all); if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent("rihou:store-changed")); }
}
