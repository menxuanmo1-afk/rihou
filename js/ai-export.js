...
import { loadAllDays, loadSettings } from "./store.js?v=62";

const REVIEW_SCIENCE = `作息规则（只用来判断，禁止在给用户的正文里写 R1、R2、「对应 Rn」、「因为 Rn」）：
- 醒后 2 小时内或午饭后刷视频/游戏 = 占意志力高峰，最差。
- 21:00 后少量消费 = 可接受放松。
- 饭后马上刷视频：血糖升叠多巴胺，下午注意力容易废。
- 大学生睡眠宜每天 8–9 小时；连续多天均 <6 小时 = 过猛欠债；入睡或起床日较差 >2 小时 = 节律乱。
- 4:00–5:00 核心体温最低、深睡窗口，晚睡补不回这一段。
- 单一学习/功课/创作连续 ≥60 分钟 = 深度块；两块之间要有 15–30 分钟真休息（不是刷手机）才能恢复注意。
- 运动较宜 14:00–18:00（体温和肌力较高）；睡前 2 小时剧烈运动容易推迟入睡。
- 末餐到睡觉宜 ≥2 小时；醒后 1 小时内无吃饭可能没吃早餐。`;
...
写短：每段最多两句；表格每格一句；禁止「洞察」小节。工作日空白 ≠ 休息（可能漏记）。不要编估值 \`$\`。给用户看的句子必须把道理写出来，例如「午饭后刷视频，下午注意力容易废」；禁止只写编号。
...
### 2. 总结
从全周里**最多提炼 3 个问题**。用编号 1. 2. 3.。每条必须是：哪天几点发生了什么 + 用白话写出作息道理（从上面规则里选一条，写成人话）。错例：「对应 R3」。对例：「周二 12:40 饭后刷视频，血糖和多巴胺一起抬高，下午注意力容易废」。不够 3 个就少写并写「没有更多问题」。硬找不算。全周都稳就只写「没有问题」。

### 3. 建议
问题有几条，建议就几条，一一对应。每条两句：改什么（落到明天或下周某日的具体钟点）+ 为什么（同样用白话写道理，禁止「因为 R3」）。不要排满一周课表。
...
### 2. 总结
只提炼 **1 或 2 个**今天最要紧的问题。每条必须是：几点发生了什么 + 用白话写出作息道理。错例：「对应 R1」。对例：「7:20 刚醒就刷视频，占掉一天里意志力最清楚的时段」。没有就写「没有问题」。禁止凑满三条。不要用一周总量来判今天。

### 3. 建议
问题有几条，建议就几条，针对今晚或明天。每条两句：做什么（带钟点）+ 为什么（白话道理，禁止规则编号）。已经过猛就写睡觉，不要加码。
