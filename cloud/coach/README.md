...
**2026-09-20 真机更新：** 最新工程已在 Xcode 打开，已签名、覆盖安装并启动到 MO-iPhone，版本 1.0（构建 2）。本机私有备份已完成，更新前后日记数据内容校验一致。下面“真机暂不可连接”是前一天接通测试时的状态，现已解决；目前待用户在手机设置中粘贴独立服务访问码、主动确认 AI 上传授权并进行真实日记验收。访问码已从 Mac 钥匙串复制到本机剪贴板供用户填写，未输出到聊天、写入 IPA 或 GitHub。没有代替用户勾选授权。
